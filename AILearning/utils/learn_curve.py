...
def get_learning_curve(start_rate, use_rate, model_generator, training_data, training_labels, test_data, test_labels):
    ...
    while used <= 1:
        ...
        training_accurancy = len([tup for tup in zip(training_predicts, use_training_labels) if tup[0] == tup[1]]) / len(use_training_labels)
        test_accurancy = len([tup for tup in zip(test_predicts, test_labels) if tup[0] == tup[1]]) / len(test_labels)
